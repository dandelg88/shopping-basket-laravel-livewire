        ...
        <ul>
            @foreach($products as $product)
                <li class="border-b border-gray-300 last:border-b-0">
                    <a class="flex items-center px-4 py-2 hover:bg-gray-200 focus:outline-none focus:bg-gray-200" href="/product/{{ $product->id }}">
                        <div class="flex-auto">
                            {{ $product->name }}
                            <span class="ml-4 text-gray-500 text-sm">${{ $product->formatted_price }}</span>
                        </div>
